...
## them.
#################################################################
TK_DIR=/usr/include/tk
TCL_DIR= /usr/include/tcl
TK_LIBS= /usr/include/tk
TK_LIBINC =  "-ltk -ltk8.6"
TCL_LIBINC = "-ltcl -ltcl8.6"
...
# Linux
SYS_LIBS= "-ldl -lcrypt -lxml2 -lpthread"
...
DBMS_INC="-I/usr/include/postgresql"
DBMS_LIB="-L/usr/lib/postgresql -lpq"
#DBMS_INC=
#DBMS_LIB=
...
all: lib/libdb.a
	...
	cd snowball; $(MAKE) SYSTYPE=$(SYSTEMTYPE) TKDIR=$(TK_DIR) TCLDIR=$(TCL_DIR) TKLIBS=$(TK_LIBS) TKLIBINC=$(TK_LIBINC) TCLLIBINC=$(TCL_LIBINC) XINC=$(X11_INCLUDES)  DBMSFLAG=$(DBMS_FLAG) DBMSINC=$(DBMS_INC) DBMSLIB=$(DBMS_LIB) ZVERSION=$(Z_VERSION); mv *.a *.o ../lib
...
clean:
	cd BerkeleyDB/build_unix; rm -rf *
	...
	cd snowball; $(MAKE) SYSTYPE=$(SYSTEMTYPE) reallyclean
