...
# Linux
SYS_LIBS= "-lxml2 -ldl -lcrypt -lpthread"
...
all: lib/libdb.a
	...
	cd index; $(MAKE) SYSTYPE=$(SYSTEMTYPE) TKDIR=$(TK_DIR) TCLDIR=$(TCL_DIR) TKLIBS=$(TK_LIBS) TKLIBINC=$(TK_LIBINC) TCLLIBINC=$(TCL_LIBINC) XINC=$(X11_INCLUDES)  ZVERSION=$(Z_VERSION)  DBMSFLAG=$(DBMS_FLAG) DBMSINC=$(DBMS_INC) DBMSLIB=$(DBMS_LIB) RANLIB=$(RANLIBPROG) libindex.a libgridindex.a ; cp lib*.a ../lib
...
bin: all
	cd utils; $(MAKE) SYSTYPE=$(SYSTEMTYPE) TKDIR=$(TK_DIR) TCLDIR=$(TCL_DIR) TKLIBS=$(TK_LIBS) TKLIBINC=$(TK_LIBINC) TCLLIBINC=$(TCL_LIBINC)  SYSLIBS=$(SYS_LIBS) XLIBS=$(X11_LIBS) XINC=$(X11_INCLUDES) DBMSLIB=$(DBMS_LIB) ZVERSION=$(Z_VERSION) all ; cp buildassoc dumprecs getnumrecs cheshire_load read_datastore cheshire_file_filter BuildContDB DumpContDB mark_recs_deleted ../bin
