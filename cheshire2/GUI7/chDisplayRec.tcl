...
proc pLabelRecord {w i format} {
    global buttonBG defaultBG defaultFont currHostName currProbType
    ...
    if {("$currHostName" == "TREC" || ("$currHostName" == "Liverpool SF"
        && $currProbType != "cs"))} {
	
	# output button text for full text window invocations
	$w insert end "  Full Text  "

	# figure out where on current line to put button tagging
	set tagstart [$w search " Full Text" $lineLoc.0]
	set pntLoc [string first . $tagstart]
	set startLoc [string range $tagstart 0 [expr $pntLoc -1] ]
	set endLoc [string range $tagstart [expr $pntLoc + 1] end]
	set endLoc [expr $endLoc + 11]
	set tagend [format %s\.%s $startLoc $endLoc]

	# add the button tag and configure appropriately
	$w tag add fullTextTag$i $tagstart $tagend

	$w tag configure fullTextTag$i -background $defaultBG \
	-relief raised -borderwidth 2 \
	-font $defaultFont

	$w tag bind fullTextTag$i <Enter> \
		"pFullTextEnter $w $i"

	$w tag bind fullTextTag$i <Leave> \
		"pFullTextLeave $w $i"

	$w tag bind fullTextTag$i <ButtonPress> \
		"pFullText $w $i"
    }
