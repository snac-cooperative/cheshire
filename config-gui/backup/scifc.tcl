...
# last modified: Tuesday 12-29-2000
# Author: Yonghui Zhang 
# Organization: SIMS, University of California, Berkeley 
#
# ---------------------------------------------------------------------------
# Description: This is the dynamic part of the code
# 
# ===========================================================================
# Implementation 
...
# filedef, indexdef, clusterdef, displaydef, compdef, and indexdef
...
proc ConfigureDbDataVariables {{path .}} {
    ...
    if [info exists dbcfg(FILEDEF${num}_8DISPOPTIONS)] {
        foreach indx $dbcfg(FILEDEF${num}_8DISPOPTIONS) { 
	...
	if {[%W nearest %y] == 1 || [%W nearest %y] == 2 || [%W \
        ...
        set dbcfg(FILEDEF${outernum}_8DISPOPTIONS) [%W curselection];
    }
}
...
proc ConfigureDbIndexVariables {{path .} {nested {}}} {
	...
	set arrPre "FILEDEF${num}_93COMPONENTS_COMPONENTDEF";
	...
	set arrPre "FILEDEF${num}_90INDEXES_INDEXDEF${indexdefNum}"; 
...
proc MoreIndexKeySpec {{nested {}}} {
	...
	set componentdefNum $appGlobal(componentdefNum);
	set compindexdefNum $appGlobal(compindexdefNum);
	set arrPre "FILEDEF${filedefNum}_93COMPONENTS_COMPONENTDEF";
	append arrPre "${componentdefNum}_COMPONENTINDEXES_";
	append arrPre "INDEXDEF${compindexdefNum}";
	set compPre "comp";
    } else {
	set indexdefNum $appGlobal(indexdefNum);
	set arrPre "FILEDEF${filedefNum}_90INDEXES_INDEXDEF${indexdefNum}"; 
	set compPre {};
    }
...
proc ConfigureIdxMapVariables {{nested {}}} {
    ...
    set arrPre "${arrPre}_4INDXMAP${idxmapNum}"
    ...
    if {[info exists dbcfg(${arrPre}_2POSIT)]} {
        $fr.positionCombo$nested entryset "$dbcfg(${arrPre}_2POSIT)"; 
    } else {
        $fr.positionCombo$nested entryset ""; 
    }

    if {[info exists dbcfg(${arrPre}_3STRUCT)]} {
        $fr.structureCombo$nested entryset "$dbcfg(${arrPre}_3STRUCT)"; 
    } else {
        $fr.structureCombo$nested entryset ""; 
    }

    if {[info exists dbcfg(${arrPre}_4RELAT)]} {
        $fr.relationCombo$nested entryset "$dbcfg(${arrPre}_4RELAT)"; 
    } else {
...
proc ConfigureDbComponentVariables {{path .}} {
    ...
    set arrPre "FILEDEF${num}_93COMPONENTS_COMPONENTDEF${componentdefNum}_";
    ...
    $parentfr.compstarttagEntry configure -textvariable \
	    dbcfg(${arrPre}3COMPONENTSTARTTAG);
...
proc ConfigureDbClusterVariables {{path .}} {
	...
	$basefr.clusbaseent configure -textvariable \
		dbcfg(FILEDEF${num}_CLUSTERS_CLUSTERDEF1_CLUSBASE);
	return;
    }
    ...
    set arrayPrefix "FILEDEF${num}_91CLUSTERS_CLUSTERDEF${clusterdefNum}"; 
    ...
    #set enum 2;
    #while {[info exists dbcfg(${arrayPrefix}_4CLUSKEY_${enum}TAGSPEC)]} {
    #    set ent [entry $thisFr.cluskeyTagspecEntry$enum \
    #            -width 60 -background gray95 \
    #            -font {-*-helvetica-medium-r-normal-*-14-*-*-*-*-*-*-*}];
    #    $ent configure -textvariable dbcfg(${arrayPrefix}_4CLUSKEY_${enum}TAGSPEC);
...
#
#    set arrPrefix "FILEDEF${filedefNum}_91CLUSTERS_CLUSTERDEF${clusterdefNum}";
#    set arrPrefix "${arrPrefix}_4CLUSKEY_";
...
proc ConfigureClusmapVariables {{context cluster}} {
    ...
    ## summarize ##
    if {$context == "cluster"} {
        set arrPre "FILEDEF${num}_91CLUSTERS_CLUSTERDEF${clusterdefNum}";
        ...
        set arrPre "FILEDEF${num}_92DISPLAYS_DISPLAYDEF${displaydefNum}";
        set arrPre "${arrPre}_CONVERT_5CLUSMAP${clusmapentryNum}"; 
    }
...
proc ConfigureClusmapFromEntryVariables {{context cluster}} {
    ...
    if {$context == "cluster"} {
        ...
        set displaydefNum $appGlobal(displaydefNum);
        set arrPre "FILEDEF${num}_92DISPLAYS_DISPLAYDEF${displaydefNum}";
        set arrPre "${arrPre}_CONVERT_5CLUSMAP${clusmapentryNum}_"; 
    }
...
proc MoreClusmapFromTagEntry {{context cluster}} {
    ...
    if {$context == "cluster"} {
        set clusterdefNum $appGlobal(clusterdefNum);
        set arrPre "FILEDEF${filedefNum}_91CLUSTERS_CLUSTERDEF${clusterdefNum}";
        set arrPre "${arrPre}_5CLUSMAP${clusmapentryNum}_";
    } elseif {$context == "display"} {    
        set displaydefNum $appGlobal(displaydefNum);
        set arrPre "FILEDEF${filedefNum}_92DISPLAYS_DISPLAYDEF${displaydefNum}";
        set arrPre "${arrPre}_CONVERT_5CLUSMAP${clusmapentryNum}_";
    }    
...
proc ConfigureDbExplainVariables {{path .}} {
    ...
    $proprietaryfr.proprietarybb1 configure \
	    -variable dbcfg(${preExp}EXPLAIN_90PROPRIETARY_NULL);
    ...
    dbcfg(${preExp}EXPLAIN_1TITLESTRING_NULL);
   
    $fr.descentry configure -textvariable appGlobal(empty)
    $fr.descentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_2DESCRIPTION_NULL);
    
    $fr.disclentry configure -textvariable appGlobal(empty)
    $fr.disclentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_3DISCLAIMERS_NULL);
   
    $fr.newsentry configure -textvariable appGlobal(empty)
    $fr.newsentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_4NEWS_NULL);
    
    $fr.hrsentry configure -textvariable appGlobal(empty)
    $fr.hrsentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_5HOURS_NULL);
   
    $fr.btentry configure -textvariable appGlobal(empty)
    $fr.btentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_6BESTTIME_NULL);

    $fr.coverentry configure -textvariable appGlobal(empty)
    $fr.coverentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_7COVERAGE_NULL);

    $fr.cprightentry configure -textvariable appGlobal(empty)
    $fr.cprightentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_8COPYRIGHTTEXT_NULL);

    $fr.cprightnoticeentry configure -textvariable appGlobal(empty)
    $fr.cprightnoticeentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_9COPYRIGHTNOTICE_NULL);
   
    ## language attributes, default to ENG
    if {![info exists dbcfg(${preExp}EXPLAIN_1TITLESTRING_LANGUAGE.a)]} {
	set dbcfg(${preExp}EXPLAIN_1TITLESTRING_LANGUAGE.a) ENG;
    }
    $fr.tslcentry configure -textvariable appGlobal(empty);
    $fr.tslcentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_1TITLESTRING_LANGUAGE.a);

    if {![info exists dbcfg(${preExp}EXPLAIN_2DESCRIPTION_LANGUAGE.a)]} {
	set dbcfg(${preExp}EXPLAIN_2DESCRIPTION_LANGUAGE.a) ENG;
    }
    $fr.desclcentry configure -textvariable appGlobal(empty);
    $fr.desclcentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_2DESCRIPTION_LANGUAGE.a);

    if {![info exists dbcfg(${preExp}EXPLAIN_3DISCLAIMERS_LANGUAGE.a)]} {
	set dbcfg(${preExp}EXPLAIN_3DISCLAIMERS_LANGUAGE.a) ENG;
    }
    $fr.disclcentry configure -textvariable appGlobal(empty);
    $fr.disclcentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_3DISCLAIMERS_LANGUAGE.a);

    if {![info exists dbcfg(${preExp}EXPLAIN_4NEWS_LANGUAGE.a)]} {
	set dbcfg(${preExp}EXPLAIN_4NEWS_LANGUAGE.a) ENG;
    }
    $fr.newslcentry configure -textvariable appGlobal(empty);
    $fr.newslcentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_4NEWS_LANGUAGE.a);

    if {![info exists dbcfg(${preExp}EXPLAIN_5HOURS_LANGUAGE.a)]} {
	set dbcfg(${preExp}EXPLAIN_5HOURS_LANGUAGE.a) ENG;
    }
    $fr.hrslcentry configure -textvariable appGlobal(empty);
    $fr.hrslcentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_5HOURS_LANGUAGE.a);

    if {![info exists dbcfg(${preExp}EXPLAIN_6BESTTIME_LANGUAGE.a)]} {
	set dbcfg(${preExp}EXPLAIN_6BESTTIME_LANGUAGE.a) ENG;
    }
    $fr.btlcentry configure -textvariable appGlobal(empty);
    $fr.btlcentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_6BESTTIME_LANGUAGE.a);

    if {![info exists dbcfg(${preExp}EXPLAIN_7COVERAGE_LANGUAGE.a)]} {
	set dbcfg(${preExp}EXPLAIN_7COVERAGE_LANGUAGE.a) ENG;
    }
    $fr.coverlcentry configure -textvariable appGlobal(empty);
    $fr.coverlcentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_7COVERAGE_LANGUAGE.a);

    if {![info exists dbcfg(${preExp}EXPLAIN_8COPYRIGHTTEXT_LANGUAGE.a)]} {
	set dbcfg(${preExp}EXPLAIN_8COPYRIGHTTEXT_LANGUAGE.a) ENG;
    }
    $fr.cprightlcentry configure -textvariable appGlobal(empty);
    $fr.cprightlcentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_8COPYRIGHTTEXT_LANGUAGE.a);

    if {![info exists dbcfg(${preExp}EXPLAIN_9COPYRIGHTNOTICE_LANGUAGE.a)]} {
	set dbcfg(${preExp}EXPLAIN_9COPYRIGHTNOTICE_LANGUAGE.a) ENG;
    }
    $fr.cprightnoticelcentry configure -textvariable appGlobal(empty);
    $fr.cprightnoticelcentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_9COPYRIGHTNOTICE_LANGUAGE.a);

    $fr.lastupdatentry configure -textvariable appGlobal(empty);
    $fr.lastupdatentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_91LASTUPDATE_NULL);

    $fr.updateintervalfr.updateintervalnuentry configure\
	    -textvariable appGlobal(empty);
    $fr.updateintervalfr.updateintervalnuentry configure -textvariable\
    dbcfg(${preExp}EXPLAIN_92UPDATEINTERVAL_1VALUE);

    # update interval units combobox 
    if {![info exists dbcfg(${preExp}EXPLAIN_92UPDATEINTERVAL_2UNITS)]} {
	$fr.updateintervalfr.updateintervalunit entryset "";
    } else {
	$fr.updateintervalfr.updateintervalunit entryset\
	$dbcfg(${preExp}EXPLAIN_92UPDATEINTERVAL_2UNITS);
    }
...
proc ConfigureDbDisplayVariables {{path .}} {
    ...
    $parentfr.formatexcludeFr.formatexcludecompressCheckbutton configure \
        -variable dbcfg(${arrPre}_EXCLUDE_COMPRESS.a);

    $parentfr.formatexcludeFr.formatexclTagspecEntry1 configure -textvariable \
        appGlobal(empty);
    $parentfr.formatexcludeFr.formatexclTagspecEntry1 configure -textvariable \
        dbcfg(${arrPre}_EXCLUDE_1TAGSPEC);
    ...
    set enum 2;
    while {[info exists dbcfg(${arrPre}_EXCLUDE_${enum}TAGSPEC)] &&
           $dbcfg(${arrPre}_EXCLUDE_${enum}TAGSPEC) != ""} {
        set ent [entry $parentfr.formatexcludeFr.formatexclTagspecEntry$enum \
                -width 60 -background gray95 \
                -font {-*-helvetica-medium-r-normal-*-14-*-*-*-*-*-*-*}];
        $ent configure -textvariable dbcfg(${arrPre}_EXCLUDE_${enum}TAGSPEC);
    ...
    if [info exists dbcfg(${arrPre}_CONVERT_FUNCTION.a)] {
	set funcName $dbcfg(${arrPre}_CONVERT_FUNCTION.a);
    ...
    for {set j 1} {$j < 6} {incr j} {
        $converFr.formatfuncradio$j \
            configure -variable dbcfg(${arrPre}_CONVERT_FUNCTION.a);
    }
    ...
    $converFr.formatfuncextfr.formatfuncextEntry configure \
        -textvariable appGlobal(empty);
    $converFr.formatfuncextfr.formatfuncextEntry configure \
        -textvariable dbcfg(${arrPre}_CONVERT_FUNCTION.a);
}
...
proc MoreFormatExcludeTagSpec {} {
    ...
    $ent configure -textvariable dbcfg(${arrPre}_EXCLUDE_${num}TAGSPEC);
...
proc DeleteAnExcludeEntry {path varPrefix} {
    ...
    set biggerNum [expr $num + 1];
    while {[info exists \
	    dbcfg(${varPrefix}_EXCLUDE_${biggerNum}TAGSPEC)]} {
	set dbcfg(${varPrefix}_EXCLUDE_${num}TAGSPEC) \
	    $dbcfg(${varPrefix}_EXCLUDE_${biggerNum}TAGSPEC);
	incr num;
	incr biggerNum;
    }
    ...
    destroy $entry$num;
    set dbcfg(${varPrefix}_EXCLUDE_${num}TAGSPEC) "";
    unset dbcfg(${varPrefix}_EXCLUDE_${num}TAGSPEC);
...
proc recWrite {{prefix {}} inlist} {
	...
	# only handle those variables that actually stored some values
		...
		# that has no further subelements)
                lappend tmparr(elements) $elename;
    ...
    # handle the leaf subelement of this tag, if any
    # for example, <FILEDEF TYPE=SGML><FILENAME>...</FILENAME><FILETAG>...
    if [info exists tmparr(elements)] {
        foreach ele $tmparr(elements) {
            append rtStr "<$ele>$dbcfg($prefix${ele})</$ele>";
        }
    }

    # now if there are further enclosed tags, handle them in turn
    # e.g, <INDEXES>, <CLUSTERS>
    set keys [array names tmparr];
    set keys [lsort -dictionary $keys];
    foreach key $keys {
        if [regexp {^(\d*[A-Z]+)(\d*)$} $key match tag num] {
            append rtStr "<$tag";
            append rtStr [recWrite "$prefix$tag${num}_" $tmparr($key)];
            append rtStr "</$tag>";
	}
    }    
    return $rtStr;
...
proc HandleComboSelect {path} {
	...
	set compindexdefNum $appGlobal(compindexdefNum);
	set idxPre \
	"FILEDEF${filedefNum}_93COMPONENTS_COMPONENTDEF${componentdefNum}";
	...
	#-----------------------------------------------------------------
	    set indexdefNum $appGlobal(indexdefNum); 
	    set idxPre \
            "FILEDEF${filedefNum}_90INDEXES_INDEXDEF${indexdefNum}"; 
	    ...
	    set indexmapNum $appGlobal(indexmapNum);
            set dbcfg(${idxPre}_4INDXMAP${indexmapNum}_ATTRIBUTESET.a) $val; 
        }
        {\.relationCombo$} {
	    set indexdefNum $appGlobal(indexdefNum); 
	    set idxPre \
            "FILEDEF${filedefNum}_90INDEXES_INDEXDEF${indexdefNum}"; 
	    set indexmapNum $appGlobal(indexmapNum);            
	    set dbcfg(${idxPre}_4INDXMAP${indexmapNum}_4RELAT) $val; 
        }
        {\.positionCombo$} {
	    set indexdefNum $appGlobal(indexdefNum); 
	    set idxPre \
            "FILEDEF${filedefNum}_90INDEXES_INDEXDEF${indexdefNum}"; 
	    set indexmapNum $appGlobal(indexmapNum);            
	    set dbcfg(${idxPre}_4INDXMAP${indexmapNum}_2POSIT) $val; 
        }
        {\.structureCombo$} {
	    set indexdefNum $appGlobal(indexdefNum); 
	    set idxPre \
            "FILEDEF${filedefNum}_90INDEXES_INDEXDEF${indexdefNum}"; 
	    set indexmapNum $appGlobal(indexmapNum);
            set dbcfg(${idxPre}_4INDXMAP${indexmapNum}_3STRUCT) $val; 
        }
        {\.truncationCombo$} {
	    set indexdefNum $appGlobal(indexdefNum); 
	    set idxPre \
            "FILEDEF${filedefNum}_90INDEXES_INDEXDEF${indexdefNum}"; 
	    set indexmapNum $appGlobal(indexmapNum);
            set dbcfg(${idxPre}_4INDXMAP${indexmapNum}_5TRUNC) $val; 
        }
        {\.completenessCombo$} {
	    set indexdefNum $appGlobal(indexdefNum); 
	    set idxPre \
            "FILEDEF${filedefNum}_90INDEXES_INDEXDEF${indexdefNum}"; 
	    set indexmapNum $appGlobal(indexmapNum);
            set dbcfg(${idxPre}_4INDXMAP${indexmapNum}_6COMPLET) $val; 
	#-----------------------------------------------------------------
	...
	#-----------------------------------------------------------------
	    ...
	    set clusterdefNum $appGlobal(clusterdefNum); 
	    set cluPre \
            "FILEDEF${filedefNum}_91CLUSTERS_CLUSTERDEF${clusterdefNum}"; 
	    set dbcfg(${cluPre}_4CLUSKEY_NORMAL.a) $val; 
        }

        {\.updateintervalunit$} {
	    set explPre "FILEDEF${filedefNum}_EXPLAIN";
            set dbcfg(${explPre}_92UPDATEINTERVAL_2UNITS) $val; 
        }

        {\.formatOIDCombo$} {
	    set displaydefNum $appGlobal(displaydefNum);
	    set dispPre \
            "FILEDEF${filedefNum}_92DISPLAYS_DISPLAYDEF${displaydefNum}"; 
...
proc HandleBBSelect {path {input ""}} {
    ...
    if {$input == ""} {
        set fileselect [tk_getOpenFile -initialdir /];
    ...
    switch -regexp -- $path { 
        {\.dataFilePathbb$} {
            set dbcfg(FILEDEF${filedefNum}_2FILENAME) $fileselect; 
        }
        {\.dataContPathbb$} {
            set dbcfg(FILEDEF${filedefNum}_filecont) $fileselect; 
        }
        {\.dataDtdbb$} {
            set dbcfg(FILEDEF${filedefNum}_3FILEDTD) $fileselect; 
        }
        {\.dataSgmlCatbb$} {
            set dbcfg(FILEDEF${filedefNum}_6SGMLCAT) $fileselect; 
        }
        {\.dataAssocPathbb$} {
            set dbcfg(FILEDEF${filedefNum}_4ASSOCFIL) $fileselect; 
        }
        {\.dataHisPathbb$} {
            set dbcfg(FILEDEF${filedefNum}_5HISTORY) $fileselect; 
        }
        {\.indexPathBrowseButton$} {
	    set indexdefNum $appGlobal(indexdefNum); 
	    set pre1 "FILEDEF${filedefNum}_90INDEXES_INDEXDEF$indexdefNum";
            set dbcfg(${pre1}_2INDXNAME) $fileselect; 
        }
        {\.indexStoplistBrowseButton$} {
	    set indexdefNum $appGlobal(indexdefNum); 
	    set pre1 "FILEDEF${filedefNum}_90INDEXES_INDEXDEF$indexdefNum";
	    set dbcfg(${pre1}_3STOPLIST) $fileselect; 
        }
        {\.compindexPathBrowseButton$} {
	    set componentdefNum $appGlobal(componentdefNum);
	    set compindexdefNum $appGlobal(compindexdefNum);
	    set pre3 "FILEDEF${filedefNum}_93COMPONENTS_COMPONENTDEF";
	    append pre3 "${componentdefNum}_COMPONENTINDEXES";
	    append pre3 "_INDEXDEF${compindexdefNum}";
            set dbcfg(${pre3}_2INDXNAME) $fileselect; 
        }
	    ...
	    append pre3 "${componentdefNum}_COMPONENTINDEXES";
	    append pre3 "_INDEXDEF${compindexdefNum}";
            set dbcfg(${pre3}_3STOPLIST) $fileselect; 
        }
	{\.compNameBButton$} {
	    set componentdefNum $appGlobal(componentdefNum);
	    set pre4 "FILEDEF${filedefNum}_93COMPONENTS_";
	    ...
	    set pre2 \
            "FILEDEF${filedefNum}_91CLUSTERS_CLUSTERDEF$clusterdefNum";
            set dbcfg(${pre2}_3STOPLIST) $fileselect; 
        }

	{\.clusbasebbut$} {
	    set \
	    dbcfg(FILEDEF${filedefNum}_CLUSTERS_CLUSTERDEF1_CLUSBASE) \
		    $fileselect;
	}
	...
	{\.formatfuncextButton$} {
	    ...
	    set dispPre \
            "FILEDEF${filedefNum}_92DISPLAYS_DISPLAYDEF$displaydefNum";
            set dbcfg(${dispPre}_CONVERT_FUNCTION.a) $fileselect;
	}
	{\.buildassocfpBB$} {
...
proc ReadOldConfigFile {} {
    ...
    set fileselect [tk_getOpenFile -initialdir [pwd]];
    if {$fileselect == ""} return;
    ...
    # also:
    # <COMPONENTSTARTTAG><ENTRY>....</ENTRY></COMPONENTSTARTTAG>
...
proc ParseOld {tag end attribute value} {
    ...
    if {$end == "/"} {
		...
		if {$appGlobal(inComponent)} {
		    append appGlobal(varPrefix) \
	            "93COMPONENTS_COMPONENTDEF${componentdefNum}_";
		...
		if {$appGlobal(inComponent)} {
		    set appGlobal(varPrefix) \
		    "FILEDEF${filedefNum}_93COMPONENTS_COMPONENTDEF";
		    ...
		    set appGlobal(varPrefix) \
                    "FILEDEF${filedefNum}_90INDEXES_INDEXDEF${indexdefNum}_";
		}
		...
		return;
    	    ...
    	    CLUSKEY
    	    {
    	        set appGlobal(inCluskey) 0;
                set appGlobal(varPrefix) \
                  "FILEDEF${filedefNum}_91CLUSTERS_CLUSTERDEF${clusterdefNum}_";
		return;
            }
		...
		set appGlobal(inFormatexc) 0;
		set appGlobal(varPrefix)\
	          "FILEDEF${filedefNum}_92DISPLAYS_DISPLAYDEF${displaydefNum}_";
		return;
	    }
	    ...
	    {
		set appGlobal(varPrefix) "FILEDEF${filedefNum}_EXPLAIN_";
		return;
	    }
	...
	# indexdef and its sub tags
		...
		set pre [append appGlobal(varPrefix)\
		    "90INDEXES_INDEXDEF${indexdefNum}_"];
	    }
        ...
        {
            # entering INDXMAP
	    set pre [append appGlobal(varPrefix) "4INDXMAP${indexmapNum}_"];
	...
	# clusterdef and its sub tags
	    ...
	    # need this state info for clusmap entry
            ...
            set pre [append appGlobal(varPrefix) \
                "91CLUSTERS_CLUSTERDEF${clusterdefNum}_"];
            # take care of attributes
	    ...
	    # need this state for the tagspec entry
            ...
            set pre [append appGlobal(varPrefix) "4CLUSKEY_"];
	...
	# displaydef and its sub tags
	    ...
	    set appGlobal(inDisplay) 1;
            set pre [append appGlobal(varPrefix) \
                "92DISPLAYS_DISPLAYDEF${displaydefNum}_"];
            # take care of attributes
            if {$attribute != ""} {
                set pairs [split $attribute " "];
                foreach pair $pairs {
	    ...
	    # need this state for the tagspec entry
            ...
            set pre [append appGlobal(varPrefix) "EXCLUDE_"];
	...
	CONVERT
	{   # entering convert
	    set pre [append appGlobal(varPrefix) "CONVERT_"];
            # take care of attributes
	    # ALL and FUNCTION
            if {$attribute != ""} {
                set pairs [split $attribute " "];
	...
	{   #entering explain
	    ...
	    append appGlobal(varPrefix) "EXPLAIN_";
	    return;
	}

	UPDATEINTERVAL
	{
	    append appGlobal(varPrefix) "92UPDATEINTERVAL_";
	    return;
	}

	PRODUCERCONTACTINFO
	{
	    append appGlobal(varPrefix) "93PRODUCERCONTACTINFO_";
	    return;
	}

	SUPPLIERCONTACTINFO
	{
	    append appGlobal(varPrefix) "94SUPPLIERCONTACTINFO_";
	    return;
	}

	SUBMISSIONCONTACTINFO
	{
	    append appGlobal(varPrefix) "95SUBMISSIONCONTACTINFO_";
	    return;
	}
	...
	COVERAGE
	{
	    ReadinHelper 7 $tag $attribute $value;
	    return;
	}
	...
	COPYRIGHTNOTICE
	{
	    ReadinHelper 9 $tag $attribute $value;
	    return;
	}
	...
	PROPRIETARY
	{
	    ...
	    set dbcfg(${pre}90PROPRIETARY_NULL) $value;
	    return;
	}
	...
	{
	    set pre $appGlobal(varPrefix);
	    set dbcfg(${pre}91LASTUPDATE_NULL) $value;
	    return;
	}
	...
	# change appGlobal(varPrefix)
	    ...
	    if {$appGlobal(inCluster)} {
                set pre "FILEDEF${filedefNum}_91CLUSTERS_";
                set pre "${pre}CLUSTERDEF${clusterdefNum}_5CLUSMAP";
                set appGlobal(varPrefix) "${pre}${clusmapentryNum}_";
            } elseif {$appGlobal(inDisplay)} {
                set pre "FILEDEF${filedefNum}_92DISPLAYS_";
                set pre "${pre}DISPLAYDEF${displaydefNum}_CONVERT_5CLUSMAP";
                set appGlobal(varPrefix) "${pre}${clusmapentryNum}_";
            }
	    return;
        }
...
proc ShowContext {} {
    ...
    set fnum 1; set idnum_1 0; set clunum_1 0; set dispnum_1 0;
    set compnum_1 0;
    foreach index $indexes {
        # try to match indexes that look like this:
        # FILEDEF1_90INDEXES_INDEXDEF1 or
        # FILEDEF3_90INDEXES_CLUSTERDEF4
	...
	if {$fdefnum > $fnum} {
            # start a new set of filedef variables
            set fnum $fdefnum;
            set idnum_$fnum 0;
            set clunum_$fnum 0;
            set dispnum_$fnum 0;
