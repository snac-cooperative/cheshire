...
from PyZ3950 import clientserver
...
class Connection(_AttrCheck, _ErrHdlr):
    ...
    def __init__(self,hostname, port, **kw):
        ...
        for (k,v) in kw.items ():
            ...
            setattr (self, k, v)
        try:
            nRS = z3950.OptionsVal.namedResultSets
            options = [nRS] # don't let user override this for now
            self._cli = clientserver.Client (hostname, port,
...
# XXX should fix: the caching here loses if the server returns fewer
# records in a present response than requested, as yaz-ztest 1.6
# seems to do regularly and is allowed by the spec.
    ...
    def _extract_recs (self, records, lbound):
        ...
        for i in range (len (recs)):
            ...
            if (typ == 'surrogateDiagnostic'):
                ...
                dat = data.encoding
                (typ, dat) = dat
                if (oid == asn1.OidVal (z3950.Z3950_USMARC)):
...
class Record:
    """Represent retrieved record.  'syntax' attribute is a string,
      ...
      OPAC     -- a hard-to-describe format (contact me if you're actually \
using this)
...
_RecordType ('USMARC', asn1.OidVal (z3950.Z3950_USMARC),
            renderer = lambda v: str(zmarc.MARC(v)))
_RecordType ('SUTRS', asn1.OidVal (z3950.Z3950_SUTRS))
_RecordType ('XML', asn1.OidVal (z3950.Z3950_XML))
_RecordType ('SGML', asn1.OidVal (z3950.Z3950_SGML))
_RecordType ('GRS-1', asn1.OidVal (z3950.Z3950_GRS1),
             renderer = lambda v: str (v),
             preproc = grs1.preproc)
_RecordType ('OPAC', asn1.OidVal (z3950.Z3950_OPAC),
             renderer = lambda v: str (v))
...
# XXX looks as if Oxford will close the connection if a delete is sent.
# Check that I correctly interpret delete bit in options resp?  Index Data
# registry says it supports delete ...
#        res.delete ()
    if query == '':
        while 1:
