...
# Version: 1.3
# Most Recent Changes: Contexts for relations, cql.within
#
...
def convertIndex(sc, top):
    ...
    if top.prefixes.has_key(set):
        ...
        for v in config.contextSetNamespaces.keys():
            if config.contextSetNamespaces[v] == setURI:
                set = v
                break
    elif not set:
        set = config.defaultContextSet
    elif not config.contextSetNamespaces.has_key(set):
        # Unknown index set
        diag = Diagnostic15()
        diag.details = set
        raise diag
...
class SearchClause (PrefixableObject):
    ...
    def toCheshire(self, top=None):
        ...
        if relation == "all":
        ...
        elif relation in ["cql.within", "cql.encloses"]:
            # Mung term if not match syntax
            t = self.term
            normal = re.compile("(\d+)o(\d+)'(\d+)''([NS]) (\d+)o(\d+)'(\d+)''([EW]) (\d+)o(\d+)'(\d+)''([NS]) (\d+)o(\d+)'(\d+)''([EW])")

            if (relation == "cql.within"):
                rel = ">#<"
            else:
                rel = "<#>"
            ...
            if normal.match(t):
                ...
                dash = re.compile("(\d+)(-(\d+))?(-(\d+))?([NS])[, ](\d+)(-(\d+))?(-(\d+))?([EW]) (\d+)(-(\d+))?(-(\d+))?([NS])[, ](\d+)(-(\d+))?(-(\d+))?([EW])")
                m = dash.match(t)
                if (m):
                    g = m.groups()
                    bits = []
                    for b in [0,2,4,5,6,8,10,11,12, 14, 16, 17, 18, 20, 22, 23]:
                        if (g[b] <> None):
                            bits.append(g[b])
                        else:
                            bits.append('0')
                    bits = tuple(bits)
                    t = "%so%s'%s''%s %so%s'%s''%s %so%s'%s''%s %so%s'%s''%s" % (bits)
                else:
                    decimal = re.compile("(-?[\d.]+) (-?[\d.]+) (-?[\d.]+) (-?[\d.]+)")
                    m = decimal.match(t)
                    if (m):
                        bits = []
                        groups = m.groups()
                        for g in range(4):
                            ...
                            if (i < 0):
                                ...
                                nsew = dir[0]
                            bits.extend([int(ad), int(am), int(as), nsew])
                        bits = tuple(bits)
                        t = "%so%s'%s''%s %so%s'%s''%s %so%s'%s''%s %so%s'%s''%s" % (bits)
                        

            clause = "%s %s {%s}" % (idx, rel, t)
        else:
            # Uhoh!
            raise(ValueError)
...
class CQLParser:
    ...
    def relation(self):
        """ Find relation """
        self.currentToken = lower(self.currentToken)
        if self.currentToken in relations or self.currentToken.find('.') > 0:
            # Step over separators
            ...
            while (self.currentToken == relationSeparator):
                ...
                if self.currentToken == relationSeparator:
                    ...
                    raise diag
                elif not self.currentToken in relationModifiers and self.currentToken.find('.') == -1:
                    diag = Diagnostic20()
                    diag.details = self.currentToken
                    raise diag
                else:
