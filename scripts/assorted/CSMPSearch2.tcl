...
puts "<BODY bgcolor=#006666 text=#FFFFFF link=#FFCC00 vlink =#FFCC99>" 
puts "<H2><IMG SRC=\"/GIFS/smallcat.gif\" \
ALIGN=left>California Sheet Music Project \
<br><em>Cheshire II Search Results</em></H2><br clear=left>"
puts "<hr>"
...
#   since INNOPAC does not handle the former
    ...
    if {$innopac == 1 && $indata(field1idx) == "topic"} {
	set indata(field1idx) "subject"
    }
    if {$innopac == 1 && $indata(field2idx) == "topic"} {
	set indata(field2idx) "subject"
    }
    if {$innopac == 1 && $indata(field3idx) == "topic"} {
	set indata(field3idx) "subject"
    }
...
# Map MELVYL A&I topic searches to the keyword index
    ...
    if {$melvyldbs == 1 && $indata(field1idx) == "topic"} {
	set indata(field1idx) "subject"
    }
    if {$melvyldbs == 1 && $indata(field2idx) == "topic"} {
	set indata(field2idx) "subject"
    }
    if {$melvyldbs == 1 && $indata(field3idx) == "topic"} {
	set indata(field3idx) "subject"
    }
    ...
    if {$AandIdb == 1 && $indata(field1idx) == "topic"} {
	set indata(field1idx) "any"
    }
    if {$AandIdb == 1 && $indata(field2idx) == "topic"} {
	set indata(field2idx) "any"
    }
    if {$AandIdb == 1 && $indata(field3idx) == "topic"} {
	set indata(field3idx) "any"
    }
...
# Map INFOBUS topic searches to the keyword index
    ...
    if {$INFOBUS == 1 && $indata(field1idx) == "topic"} {
	set indata(field1idx) "subject"
    }
    if {$INFOBUS == 1 && $indata(field2idx) == "topic"} {
	set indata(field2idx) "subject"
    }
    if {$INFOBUS == 1 && $indata(field3idx) == "topic"} {
	set indata(field3idx) "subject"
    }
...
# Map topic to kw on other hosts
    ...
    if {$OtherKw == 1 && $indata(field1idx) == "topic"} {
	set indata(field1idx) "any"
    }
    if {$OtherKw == 1 && $indata(field2idx) == "topic"} {
	set indata(field2idx) "any"
    }
    if {$OtherKw == 1 && $indata(field3idx) == "topic"} {
	set indata(field3idx) "any"
    }

# Map topic to subject where necessary

    if {$servername == "LC_BOOKS" && $indata(field1idx) == "topic"} {
	set indata(field1idx) "subject"
    }
    if {$servername == "LC_BOOKS" && $indata(field2idx) == "topic"} {
	set indata(field2idx) "subject"
    }
    if {$servername == "LC_BOOKS" && $indata(field3idx) == "topic"} {
	set indata(field3idx) "subject"
    }
...
# Determine the number of indexes
   ...
   if {$indata(field1cont) != ""} {
       incr numindexes
       append CURRENT_TERMS " $indata(field1cont)"
       regsub -all -nocase { and } $indata(field1cont) " " indata(field1cont);
   }
   if {$indata(field2cont) != ""} {
       incr numindexes
       append CURRENT_TERMS " $indata(field2cont)"
       regsub -all -nocase { and } $indata(field2cont) " " indata(field2cont);
   }
   if {$indata(field3cont) != ""} {
       incr numindexes
       append CURRENT_TERMS " $indata(field3cont)"
       regsub -all -nocase { and } $indata(field3cont) " " indata(field3cont);
   }
   ...
   if {$numindexes == 0} {
       ...
       if {$indata(field1idx) != "none" && $indata(field1cont) != ""} {
	   if {($servername == "cheshire" || $servername == "SUNSITE"
	   || $servername == "UCBDL") || $servername == "CSMP"
	   && ($indata(field1idx) == "topic" || $indata(field1idx) == "any")} {
	       append query "($indata(field1idx) @ \{$indata(field1cont)\} )"
	   } else {
	       append query "($indata(field1idx)  \{$indata(field1cont)\} )"
	   }
	   set firstterm 0
       }
       if {$indata(field2idx) != "none" && $indata(field2cont) != ""} {
	   if {$firstterm == 0} {
	       append query " $indata(boolean) "
	   }
	   if {($servername == "cheshire" || $servername == "SUNSITE"
	   || $servername == "UCBDL") || $servername == "CSMP"
	   && ($indata(field2idx) == "topic" || $indata(field2idx) == "any")} {
	       append query "($indata(field2idx) @ \{$indata(field2cont)\} )"
	   } else {
	       append query "($indata(field2idx) \{$indata(field2cont)\} )"
	   }
	   set firstterm 0
       }
       if {$indata(field3idx) != "none" && $indata(field3cont) != ""} {
	   if {$firstterm == 0} {
	       append query " $indata(boolean) "
	   }
	   if {($servername == "cheshire" || $servername == "SUNSITE"
	   || $servername == "UCBDL") || $servername == "CSMP"
	   && ($indata(field2idx) == "topic" || $indata(field2idx) == "any")} {
	       append query "($indata(field3idx) @ \{$indata(field3cont)\} )"
	   } else {
	       append query "($indata(field3idx) \{$indata(field3cont)\} )"
	   }
	   set firstterm 0
       }
       if {$servername == "CSMP" && $indata(langidx) != "none"} {
	   puts "passed"
	   if {$firstterm == 0} {
	       append query " and "
	   }
	   append query "(language $indata(langidx))"
       }
...
if {$servername != "UCBDL" && $servername != "CSMP"} {
    if {$INFOBUS} {
...
while {($numdisp < $maxnum) && ($numrecv > 0)} {
	...
	if {$servername == "ATT_CIA"} {
	    ...
	    puts "<p><B>Record #$num</B>:"
	    
	    set outrec0 [zformat $fmt $y 2 $num 80 $DTD]
	    
	    regsub -all {lctgm} $outrec0 {(Image)} outrec1
	    regsub -all {pht} $outrec1 {(Photographer)} outrec2
	    regsub -all {prf} $outrec2 {(Portrait)} outrec1
	    
	    if {[regexp -nocase {(http://www.mip.berkeley.edu/cgi-bin/csmp)([?])([0-9]*)} $outrec1 all addr qmark idnumber] >= 1} {
		set purlflag 0
		set addr "/cgi-bin/outtest.tcl"
		regsub -all {<strong><A HREF=.*</A>} $outrec1 "<FORM ACTION=$addr METHOD=POST><INPUT TYPE=HIDDEN NAME=IMAGEID VALUE=\"$idnumber\"> <INPUT TYPE=SUBMIT VALUE=\"View Page Images\"> " outrec2

	    } else { # it is probably a old PURL record
		set purlflag 1
		regsub -all {<strong><A HREF=.*</A>} $outrec1 {<i> Page images not available.</i>}  outrec2
	    }
	    puts "$outrec2"
	    
	    if {$purlflag == 0} {
		# now do some different substitutions for the item being sent 
		regsub -all {<A HREF=.*</A>} $outrec1 "<b>Record ID #$idnumber</b>" outrec2
		set newline [format "\n"]
		regsub -all $newline $outrec2 { } outrec1
		regsub -all {["]} $outrec1 {''} outrec3

		puts "<INPUT TYPE=HIDDEN NAME=BIBRECORD VALUE=\"$outrec3\" >"
		puts "</FORM>"
            }
...
if {$nextrec <= $hits} {
    puts "<HR>"
    puts "<FORM \
ACTION=\"/cgi-bin/CSMPSearch.tcl\" METHOD=POST>"
