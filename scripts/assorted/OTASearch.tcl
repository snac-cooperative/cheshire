...
puts "<BODY bgcolor=#446688 text=#FFFFFF link=#FFCC00 vlink =#FFCC99>" 
puts "<H2><IMG SRC=\"/GIFS/smallcat.gif\" \
ALIGN=left>Oxford Text Archive Test Collection \
...
# The other regsubs have been added as problems cropped up.
   ...
   if {$servername == "OTA"} {
       ...
       set CHESHIRE_CONFIGFILE "/usr6/ray/OTA/CONFIG.OTA"
...
# Determine the number of indexes
   ...
   if {$numindexes == 0} {
	   ...
	   || $servername == "UCBDL") || $servername == "OTA"
       ...
       if {$servername == "OTA" && $indata(langidx) != "none"} {
	   puts "passed"
	   if {$firstterm == 0} {
	       append query " and "
	   }
...
switch $servername {
    ...
    "OTA" {set serverdisp "Oxford Text Archive Test Collection"}
...
if {$servername != "UCBDL" && $servername != "OTA"} {
    if {$INFOBUS} {
...
if {$servername != "UCBDL" && $servername != "OTA"} {
   ...
   set numdisp 0
	...
	set DTD "/usr6/ray/OTA/us_marc.dtd"
	set rectype 2
    }
}
...
while {$numdisp < $maxnum && $numrecv > 0} {
	...
	if {$servername == "ATT_CIA"} {
	    ...
	    puts "<p>"	    
	} elseif {$servername == "OTA"} {
	    # Must format the html for the dl display here
	    puts "<p><B>Record #$num</B>:"

	    set outrec0 [zformat $fmt $y 2 $num 80 $DTD]
	    # change the "SUB856MESSAGE" field to something else
	    regsub -all {SUB856MESSAGE} $outrec0 "Click here for page images" outrec1
	    regsub -all {Composer} $outrec1 {Author} outrec2
	    regsub -all {lctgm} $outrec2 {(Image)} outrec1
	    regsub -all {prf} $outrec1 {(Portrait)} outrec2
	    regsub -all {pht} $outrec2 {(Photographer)} outrec1
...
if {$nextrec <= $hits} {
    puts "<HR>"
    puts "<FORM \
ACTION=\"/cgi-bin/OTASearch.tcl\" METHOD=POST>"
