...
#
# ***********************************************************************/
proc mkBibDel {{w .bibdel}} {

	global fieldlist 
	catch {destroy $w}
	toplevel $w
	
	wm title $w "Bibliographic Record Processing: Deletion"
	...
	pack [label $w.emptylabeltop -text "          "] \
		...
		entries to retrieve records for deletion. \nPress \
		<Return> or click \
	...
	foreach i $fieldlist {
		...
		bind $w.searchentry$i <Return> {destroy .bibdel
						wm withdraw .aubib
						mkBibSearchProcessDel}
	}

	pack [button $w.ok -text "OK" -relief raised -bd 2 \
		-command {destroy .bibdel
			 wm withdraw .aubib
			 mkBibSearchProcessDel}] \
