...
#
# ***********************************************************************/
proc mkBibLongDisplayDel {{w .dis2}} {
	...
	wm title $w "MARC Record Deletion Panel"
	wm geometry $w +0+0
   ...
   button $w.frame3.ok -text "Delete Me" -command {destroy .dis2
				set result ""
			        doBibDel $recno
        			mkMsgDialog .sucessmsg \
				  	{-text "****** Completion \
						****** \n\nMARC \
                				record processing \
                				done sucessfully.\n \
						...
						-aspect 250 \
                				-justify left} \
					{Quit {focus .nameentry; \
						wm deiconify .}} \
                			{{Processing Another Records} \
						{wm deiconify \
                        				.aubib}}
   ...
   button $w.frame3.quit -text "Quit to Login Screen" \
    ...
    message $w.frame1.m -font -Adobe-Times-Medium-R-Normal-*-180-* \
        -aspect 1200 \
        -text "In this panel, you can look the MARC record using the \
                following methods: \n \
                \t1. press <Button 1> on scrollbar, or, \
			press <Button 2> to scroll screens, \n \
                \t2. <tab> through different field entries, \n \
                \t3. to confirm the deletion, \
		click on the <Delete Me> button on the bottom of screen......"
