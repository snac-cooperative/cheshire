...
if [catch {open ${run_name}_all w} outfile] {
...
foreach query [array names q_titles] {
    set q_string {}
    append q_string "{" $q_titles($query) " " $q_desc($query) " " $q_narr($query) "}"
puts "query content is '$q_string'"
    ...
    foreach rec $results {
        ...
        puts $outfile "$query 0 $docno $rank $sim ${run_name}_all"
        incr rank
        incr sim -1
    }
...
exec sort -k 1.n,1 -k 4.n,4 ${run_name}_all > result_tmp
exec mv result_tmp ${run_name}_all
