...
if [catch {open ${run_name}_ti_de w} outfile] {
...
foreach query [array names q_titles] {
    set q_string {}
    append q_string "{" $q_titles($query) " " $q_desc($query) "}"
puts "query content is '$q_string'"
    ...
    foreach rec $results {
        ...
        puts $outfile "$query 0 $docno $rank $sim ${run_name}_ti_de"
        incr rank
        incr sim -1
    }
...
exec sort -k 1.n,1 -k 4.n,4 ${run_name}_ti_de > result_tmp
exec mv result_tmp ${run_name}_ti_de
