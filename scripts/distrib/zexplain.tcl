...
proc process_explain_attributedetails {item} {
	...
	# These are the "legal" combinations of attributes for searching
	# puts "<b>Attribute Combinations</b>:"
	set defaultattributeset [lindex [lindex [lindex $item 1] 0] 1]
	#puts "<b> Default AttributeSet</b>: $defaultattributeset"
	set COMBOS($currdbname..DEFAULTATTR) $defaultattributeset

	#puts "NEW is [lindex [lindex $item 1] 1]"
	foreach combo [lrange [lindex [lindex $item 1] 1] 1 end] {
	    #puts "COMBO IS $combo"
	    process_explain_attributedetails $combo 
	}
    } elseif {$title == "AttributeCombination"} {
	# This is a set of attributes and values...
	set comblist {}
	foreach attr [lrange $item 1 end] {
	    set type [lindex [lindex $attr 1] 1]
	    set required [lindex [lindex $attr 2] 1]
	    set code [lindex [lindex $attr 3] 0]
	    set val  [lindex [lindex $attr 3] 1]
	    if {$code != "any_or_none::"} {
		lappend comblist "$type=$val"
		#puts "COMBLIST $type=$val"
	    }
	}
	lappend COMBOS($currdbname) $comblist
	...
	lappend INDEXES($currdbname) [lrange $item 1 end]
	...
	if {[llength $values] > 1} {
		...
		lappend INDEXES($currdbname) "$desc $value"
	    }
	    #puts "</table>"
	}
...
proc process_explain_databaseInfo {item index has_title do_commoninfo} {
    global currdbname URI_HOST URI_PORT currrecordcount
...
proc search_explain_databaseinfo {} {
    global URI_HOST URI_PORT currdbname searchterms currrecordcount
    global DATABASES 
   ...
   #puts "<html>\n<head>\n<title>Databases on z39.50 server: $URI_HOST</title>\n</head>\n<body bgcolor = white>\n<center><h3>Databases on z39.50 server: $URI_HOST</h3></center>\n<p>"
	...
	lappend DATABASES "$currdbname $currrecordcount"
