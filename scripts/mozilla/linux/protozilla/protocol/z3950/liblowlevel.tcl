# ------------  Low Level Routines  -------------
# ( Or, more accurately, boring routines )
#
...
# Hehe, cute history js trick
proc exit_error {reason} {
    global  HTML_RESULT searchterms

    set HTML_RESULT "<html><head><script>alert(\"$reason\");</script></head><body><script>window.history.back();</script></body></html>"
    throw_result
}
...
proc generate_scan_query {} {
	...
	} elseif {$name == "scanterm" && $searchterms(scanterm) != ""} {
	    set scanterm $searchterms($name)
	}
    }
...
proc generate_sort_query {} {
    global searchterms newsetname
    set sortcmd "zsort "

    append sortcmd " -IN {"
    set terms [split $searchterms(sets) ","]
    set newset ""
    foreach t $terms {
	append sortcmd "$t "
	append newset "$t+"
    ...
    append sortcmd "}"
    set newset [string range $newset 0 [expr [string length $newset]-2]]

    set newsetname $newset

    append sortcmd " -OUT $newset"

    if { [lsearch [array names searchterms] "sortfield"] != -1 } {
	append sortcmd " -ATTR $searchterms(sortfield)"
    } else {
	append sortcmd " -TAG $searchterms(sortkey)"
    }

    if { $searchterms(descend) == "true" } {
	append sortcmd " -DESCENDING"
    }
    if { $searchterms(case) == "false" } {
	append sortcmd " -IGNORE_CASE"
    }
...
proc process_query_string {} {
    ...
    set searchterms(resultsetname) ""
    set searchterms(sidebar) false
...
proc build_sort_navbar {} {
    global searchterms FIRST_RECORD NUM_RECORDS URI_HOST URI_PORT DATABASENAME HITS RECORD_FORMAT RECV_RECORDS
    
    set prev "<form action = \"z3950://$URI_HOST:$URI_PORT/sort/$DATABASENAME\" METHOD=\"GET\">"
    set next "<form action = \"z3950://$URI_HOST:$URI_PORT/sort/$DATABASENAME\" METHOD=\"GET\">"
    
    # Get relevant parts of searchterms array
    
    set inlist [array names searchterms]
    
    if {[lsearch $inlist "sortfield"] != -1 || [lsearch $inlist "sortkey"] != -1} {
	# Simple sort

	if {[lsearch $inlist "sortfield"] != -1} {
	    append prev "<input type=\"hidden\" name=\"sortfield\" value = \"$searchterms(sortfield)\">"
	    append next "<input type=\"hidden\" name=\"sortfield\" value = \"$searchterms(sortfield)\">"
	}  else {
	    append prev "<input type=\"hidden\" name=\"sortkey\" value = \"$searchterms(sortkey)\">"
	    append next "<input type=\"hidden\" name=\"sortkey\" value = \"$searchterms(sortkey)\">"
	}

	append prev "<input type=\"hidden\" name=\"descend\" value = \"$searchterms(descend)\">"
	append prev "<input type=\"hidden\" name=\"case\" value = \"$searchterms(case)\">"
	append prev "<input type=\"hidden\" name=\"sets\" value = \"$searchterms(sets)\">"

	append next "<input type=\"hidden\" name=\"descend\" value = \"$searchterms(descend)\">"
	append next "<input type=\"hidden\" name=\"case\" value = \"$searchterms(case)\">"
	append next "<input type=\"hidden\" name=\"sets\" value = \"$searchterms(sets)\">"
...
proc build_cgihref {recno} {
    ...
    if { [llength [array names fields]] == 0} {
	# Sort

	set href "z3950://$URI_HOST:$URI_PORT/sort/$DATABASENAME?"

	
	if { [lsearch [array names searchterms] "sortfield"] != -1 } {
	    append href "&sortfield=$searchterms(sortfield)"
	} else {
	    append href "&sortkey=$searchterms(sortkey)"
	}
	append href "&case=$searchterms(case)&descend=$searchterms(descend)&sets=$searchterms(sets)"
    } else {
	foreach name [array names fields] {
	    set value $fields($name)
	    if { $value != "" } {
