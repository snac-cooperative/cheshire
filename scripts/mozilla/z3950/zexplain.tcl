...
proc process_explain_attributedetails {item} {
	...
	append HTML_RESULT "<b>Attribute Set</b>: [lrange $item 1 end]<br>"
...
proc do_explain_categoryInfo {title} {
    global URI_HOST URI_PORT explain_database HTML_RESULT QUERY 
    ...
    set results [try_zsearch $explain_database 1]
...
proc search_explain_attributedetails {} {
    global URI_HOST URI_PORT HTML_RESULT QUERY explain_database
...
proc search_explain_indexes {} {
    global URI_HOST URI_PORT database attr_values attr_descs HTML_RESULT QUERY

    # Need to catch all this and check for non exp1 servers
    # Then check our pseudo-database
    # Then fall back to strobe

    if { [try_zconnect "IR-Explain-1"] } {

	zset attributes explain
	zset recsyntax explain
	set QUERY "zfind explainCategory AttributeDetails AND databasename $database"

	set results [try_zsearch "IR-Explain-1" 0]
	...
	if {$results != 0} {
	    ...
	    # Need to step through till we find attributesByType and get type 1
	    ...
	    throw_file "index_explain_template.html"
	    
	    set url "z3950://$URI_HOST:$URI_PORT/search/$database"
...
<option value = \"PHON\"> phonetically like </option> \
 <option value = \"STEM\"> for (stemmed value) </option>"
	    ...
	    append formtxt "<tr><td>First Record:</td><td><input type = \"text\" name = \"numreq\" size = 10 value = 1></td></tr>"
...
proc search_explain_targetinfo {} {
    global HTML_RESULT QUERY database
    ...
    set sresults [try_zsearch $database 1]
...
proc search_explain_recordsyntaxinfo {} {
    ...
    set results [try_zsearch $database 1]
...
proc search_explain_databaseinfo {} {
    global URI_HOST URI_PORT currdbname searchterms HTML_RESULT QUERY database
...
proc search_explain_type {type} {
    global URI_HOST URI_PORT database QUERY
