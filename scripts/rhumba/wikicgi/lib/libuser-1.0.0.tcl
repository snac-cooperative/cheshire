...
proc rebuild_user_database {} {
  ...
  if { $REBUILD_LEVEL_USERS  == 2 } {
	...
	regsub -all "\n" $results "<br>" results
	puts "$results"
	throwfile "err.html" [list "\%ERR\% Could not rebuild the editor database! The system needs manual rebuild immediately, contact an administrator.<p>Errors given:<p>$results</p>"]
	return 0
    } else {
	return 1
    }
